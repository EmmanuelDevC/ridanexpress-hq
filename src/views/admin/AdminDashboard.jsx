...
const AdminDashboard = () => {
        ...
        <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 p-4 rounded-xl border border-slate-700 hover:border-indigo-400 transition-all duration-300 shadow-lg hover:shadow-indigo-500/20">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <span className={`text-xs ${trend.includes('+') ? 'text-emerald-400' : 'text-rose-400'} flex items-center gap-1 mt-1`}>
                        <span className={`inline-block w-0 h-0 border-x-4 border-x-transparent ${trend.includes('+') ? 'border-b-[6px] border-b-emerald-400' : 'border-t-[6px] border-t-rose-400'}`}></span>
    ...
    return (
        <div className="px-2 md:px-4 lg:px-6 py-4 bg-gradient-to-br from-slate-900 to-gray-900 min-h-screen">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3 mt-1 flex-wrap">
                                    <p className="text-xs text-slate-400">
                                        Last login: {moment().format('MMM D, YYYY h:mm A')}
                                    </p>
                ...
                <div className='lg:w-5/12 bg-gradient-to-br from-slate-800/50 to-slate-900/50 p-4 md:p-6 rounded-2xl border border-slate-700 shadow-xl'>
                    ...
                    <div className='space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar'>
                        {recentMessage.map((m, i) => (
                            <div key={i} className='flex items-start gap-3 p-4 bg-slate-800/30 rounded-xl border border-slate-700 hover:border-indigo-500/50 transition-all'>
                                <div className='relative flex-shrink-0'>
                                    {m.senderId === userInfo._id ?
                                        ...
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-r from-amber-600 to-orange-500 flex items-center justify-center shadow-lg">
                                            <img className='w-6 h-6' src={seller} alt="Seller" />
                                        </div>
                                    }
                        ...
                        <tbody className="text-slate-300">
                            {recentOrders.map((d, i) => (
                                <tr key={i} className="border-b border-slate-700 hover:bg-slate-800/10 transition-colors">
                                    ...
                                    <td className="p-3">
                                        <span className={`px-2 py-1 rounded-full text-xs ${d.payment_status === 'completed'
                                    ...
                                    <td className="p-3">
                                        <span className={`px-2 py-1 rounded-full text-xs ${d.delivery_status === 'delivered'
