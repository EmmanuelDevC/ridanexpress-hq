...
const Sidebar = ({ showSidebar, setShowSidebar }) => {
  ...
  return (
    <div>
      ...
      <div className={`w-[260px] fixed bg-gray-900 rounded-r-[1rem] border-r border-indigo-500 z-50 top-0 h-screen shadow-[0_0_15px_0_rgb(34_41_47_/_5%)] transition-all ${showSidebar ? 'left-0' : '-left-[260px] lg:left-0'}`}>
          ...
          <ul>
            {
              allNav.map((n, i) => <li key={i}>
                <Link to={n.path} className={`${pathname === n.path ? 'bg-indigo-600/20 text-indigo-400 shadow-indigo-500/30 duration-500 ' : 'text-[#d0d2d6] font-normal duration-200'} px-[12px] py-[9px] rounded-xl flex justify-start items-center gap-[12px] hover:text-gray-300 hover:bg-gray-800 hover:pl-4 transition-all w-full mb-1 `}>
                  <span>{n.icon}</span>
                  <span>{n.title}</span>
                </Link>
              </li>)
